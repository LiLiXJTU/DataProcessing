...
def dcm2nii_CT(CT_dcm_path, nii_out_path):
    ...
    with tempfile.TemporaryDirectory() as tmp: #convert CT
        ...
        path = nii_out_path + '/CT.nii.gz'
        shutil.copy(nii, path)
...
def dcm2nii_PET(PET_dcm_path, nii_out_path):
    ...
    with tempfile.TemporaryDirectory() as tmp: #convert PET
        ...
        shutil.copy(nii, PET_path)

        # convert pet images to quantitative suv images and save nifti file
        suv_pet_nii = convert_pet(nib.load(PET_path), suv_factor=suv_corr_factor)
        # nib.save(suv_pet_nii, nii_out_path/'SUV.nii.gz')
        SUV_path = nii_out_path + '/SUV.nii.gz'
        nib.save(suv_pet_nii, SUV_path)
...
def dcm2nii_mask(mask_dcm_path, nii_out_path):
    ...
    path = str(nii_out_path + '/PET.nii.gz')
    pet = nib.load(path)
...
def convert_tcia_to_nifti(study_dirs,nii_out_root):
    ...
    for study_dir in tqdm(study_dirs):
        ...
        nii_out_path = nii_out_root +'/' + study_dir.parent.name
...
if __name__ == "__main__":
    ...
    path_to_data = r'D:\data\FDG-PET-CT-Lesions\manifest-1654187277763\FDG-PET-CT-Lesions/'
    nii_out_root = r'D:\data\FDG-PET-CT-Lesions\manifest-1654187277763\tcia_nifti\FDG-PET-CT-Lesions/'
